...
class TrimapSynthesizer:
    ...
    def _compute_structural_core(self, dist_inside: np.ndarray) -> np.ndarray:
        """
        Compute the structural core (medial axis) to protect object structure.
        
        Finds the "ridges" of the distance transform (local maxima) which represent
        the skeleton of the object. This ensures that even with aggressive erosion,
        the central structure (bones of the matte) is preserved.
        """
        import cv2
        
        # Find local maxima in distance transform
        # A pixel is a ridge if it's equal to its dilated neighbors
        kernel_size = 5
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
        dilated_dist = cv2.dilate(dist_inside, kernel)
        
        # Ridges are peaks (where value equals dilated value)
        # Filter low values to avoid noise at edges
        min_width = 10.0 # Minimum distance from edge to be considered "structure"
        ridges = (dist_inside == dilated_dist) & (dist_inside > min_width)
        
        skeleton = ridges.astype(np.uint8) * 255
        
        # Dilate the skeleton to give it "bone width"
        # This ensures we keep a solid core around the skeleton
        bone_thickness = 15 # Pixels radius to protect around skeleton
    ...
    def _assemble_trimap(
        ...
        # 1. Create Adaptive Core (based on edge complexity)
        ...
        # 2. Create Structural Core (based on matte center/skeleton)
        # Protects the "bones" of the matte (shoulder, arm, head) from over-erosion
        structural_core = self._compute_structural_core(dist_inside)
        
        # Combine: Core is valid if it satisfies Adaptive OR Structural criteria
        core_mask = cv2.bitwise_or(adaptive_core, structural_core)
        
        # Ensure core never exceeds original mask (sanity check)
        core_mask = cv2.bitwise_and(core_mask, (mask_binary * 255).astype(np.uint8))
